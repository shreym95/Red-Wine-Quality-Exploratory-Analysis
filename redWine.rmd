...
ggplot(aes(x = quality, y = volatile.acidity), data = rdw)+
  geom_boxplot(alpha = 1/4, color = "#031b4c")+
  geom_jitter(alpha = 1/4, color = "#37768e")+
  stat_summary(geom = "point", fun.y = "mean", shape = 3, size = 4, color = "red")
```

From the median and mean points, we clearly see a declining relationship with quality. And normal quality values are packed mostly in the 0.2 to 0.8 range. Bear in mind that this is a jitter plot, with noise. Let's make some other line plots.

```{r echo=FALSE}
ggplot(aes(y = volatile.acidity, x = quality), data = rdw)+
  geom_jitter()
```
